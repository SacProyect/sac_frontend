                ...
                <div>
                    <div id="supervisor-table" className="h-[400px] overflow-y-auto custom-scroll p-4">
                        <div className="space-y-4">
                            {supervisorData.map((group, index) => (
                                <div key={index} className="border border-[#3a3a39] rounded-lg p-4">
                                    <h3 className="mb-3 font-semibold text-blue-400">{group.group}</h3>

                                    {/* Mejor supervisor */}
                                    <div className="p-3 mb-2 border rounded-md bg-green-900/20 border-green-700/30">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Award className="w-4 h-4 text-green-400" />
                                            <span className="font-medium text-green-400">Mejor: {group.best.name}</span>
                                        </div>
                                        <div className="grid grid-cols-4 gap-2 text-sm">
                                            <div>
                                                <span className="text-gray-400">IVA:</span>
                                                <div className="font-medium">{formatCurrency(group.best.iva)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">ISLR:</span>
                                                <div className="font-medium">{formatCurrency(group.best.islr)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">Multas:</span>
                                                <div className="font-medium">{formatCurrency(group.best.multas)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">Total:</span>
                                                <div className="font-bold text-green-400">{formatCurrency(group.best.total)}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-3 border rounded-md bg-red-900/20 border-red-700/30">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="font-medium text-red-400">Menor: {group.worst.name}</span>
                                        </div>
                                        <div className="grid grid-cols-4 gap-2 text-sm">
                                            <div>
                                                <span className="text-gray-400">IVA:</span>
                                                <div className="font-medium">{formatCurrency(group.worst.iva)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">ISLR:</span>
                                                <div className="font-medium">{formatCurrency(group.worst.islr)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">Multas:</span>
                                                <div className="font-medium">{formatCurrency(group.worst.multas)}</div>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">Total:</span>
                                                <div className="font-bold text-red-400">{formatCurrency(group.worst.total)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
