...
/** Backend devuelve { data, total, page, totalPages, limit }. */
export const getTaxpayers = async (): Promise<TaxpayersListResponse> => {
	try {
		const requestURL = "/taxpayer/get-taxpayers";
		const response = await apiConnection.get(requestURL);
		const body = response.data as TaxpayersListResponse;
		return body;
	} catch (e) {
		console.error(e);
...
/** Backend devuelve { data, total, page, totalPages, limit }. */
export const getTaxpayerForEvents = async (): Promise<TaxpayersListResponse> => {
	try {
		const requestURL = "taxpayer/get-taxpayers-for-events";
		const response = await apiConnection.get(requestURL);
		return response.data as TaxpayersListResponse;
	} catch (e) {
		console.error(e);
